    ...
    def decrypt(self, text):
        ...
        while 1:
            ...
            if parentscore > maxscore:
                ...
                decrypted_text = ss.decrypt(text)
                print(f'best key: {maxkey}')
                print(f'plaintext: {decrypted_text}')
                print(f'accuracy: {self._are_words_percentage(decrypted_text)}%')
...
if __name__ == "__main__":
    sc = SubstitutionCypher()
    sb = SubstitutionBreaker()
    
    message = input('Type text to encode: ')
    encrypted_message = sc.encrypt(message.strip())
